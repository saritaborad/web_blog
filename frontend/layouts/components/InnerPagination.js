...
const InnerPagination = ({ posts, date }) => {
 ...
 const prevButton = prev && (
  <Link href={prev} className={"cat-btn bg-button dark:bg-darkmode-button"} style={{ border: "none", borderRadius: "20px" }}>
   Prev
  </Link>
 );
 const nextButton = next && (
  <Link href={next} className={"cat-btn bg-button dark:bg-darkmode-button"} style={{ border: "none", borderRadius: "20px" }}>
   Next
  </Link>
 );
