    ...
    <div className="card-tags">
     {post.categories?.length > 0 &&
      post.categories.map((tag, i) => (
       <Link key={i} className="capitalize card-tag" href={`/categories/${tag.name.replace(" ", "-")}`}>
        {tag.name}
       </Link>
      ))}
    </div>
    <Link href={`/posts/${post.slug}`} className="card-txt">
     {post.title}
    </Link>
