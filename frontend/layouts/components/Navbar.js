   ...
   <ul className={`md:flex md:items-center md:pb-0 pb-12 absolute md:static md:z-auto z-[2] left-0 w-full md:w-auto md:pl-0 pl-9 transition-all duration-500 ease-in ${open ? "nav-open mt-0" : "top-[-490px] bg-transparent"}`}>
    {Links.map((link) => (
     <li key={link.name} className={`${link.name === "Categories" ? "xxs:block md:hidden md:ml-8 text-xl md:my-0 my-7 " : "md:ml-8 text-xl md:my-0 my-7 "}`}>
      <Link href={link.link} className={router?.asPath === `${link.link}` ? `nav-item active md:py-11 md:px-5 dark:text-primary` : `nav-item dark:text-primary`}>
       {link.name}
      </Link>
     </li>
    ))}
    <ThemeSwitcher />
    <Link href="/contact" className="nav-btn text-white px-6 py-4 rounded md:ml-8 md:flex md:hidden dark:bg-darkmode-primary">
     Get In Touch
    </Link>
   </ul>
   <Link href="/contact" className="nav-btn text-white px-6 rounded md:ml-8 md:flex hidden dark:bg-darkmode-primary">
    Get In Touch
   </Link>
  </div>
 );
